...
export async function getQrCodeOr404(req, res, checkDomain = true) {
    try {
        ...
        if (
            response === undefined ||
            (checkDomain && (await getShopUrlFromSession(req, res)) !== response.shopDomain)
        ) {
            res.status(404).send();
        } else {
            return response;
...
export async function formatQrCodeResponse(req, res, rawCodeData) {
    ...
    */
    const formattedData = rawCodeData.map((qrCode) => {
        ...
        /*
            TODO: A user might create a QR code with a discount code and then later delete that discount code.
